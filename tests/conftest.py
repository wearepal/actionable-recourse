...
def recourse_builder(request, classifier, action_set):
    ...
    rb = RecourseBuilder(solver=request.param, action_set=action_set, clf=classifier)
...
def recourse_builder_cpx(classifier, action_set):
    action_set.set_alignment(classifier)
    rb = RecourseBuilder(solver=_SOLVER_TYPE_CPX, action_set=action_set, clf=classifier)
