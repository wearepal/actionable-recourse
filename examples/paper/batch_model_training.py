...
if settings["train_classifiers"]:
    ...
    if settings["method_name"] == "logreg":
        ...
        param_grid = {
            "C": [1.0 / l for l in [0.01, 0.1, 1, 10, 100]],
            "dual": [False],
            "tol": [1e-8],
        }
    else:
    ...
    model_stats_df = format_gridsearch_df(
        grid_search_df,
        settings=settings,
...
if save_to_disk(file_names["coef_table"]):
    if settings["method_name"] == "logreg":
        coef_df = coef_df.rename(
            columns={col: 1.0 / float(col.split("_")[1]) for col in coef_df.columns}
        )
...
if save_to_disk(file_names["model_size"]):
    ...
    non_zero_sum_actionable = nnz_coef_df.pipe(
...
if save_to_disk(file_names["coef_path"]):
    f, ax = create_coefficient_path_plot(
        coefs_df=coef_df, fig_size=(20, 20), label_coefs=True, fontsize=12
    )
    ax = fix_font_sizes(ax)
