...
X = data_df.drop("GoodCustomer", axis=1).drop(
...
if not use_cached_flipset:
    ...
    for i in denied_individuals:
        ...
        fb = RecourseBuilder(
            ...
            intercept=intercept - (np.log(p / (1.0 - p))),
...
matching_df["y_pred_bin"] = pd.cut(matching_df["y_pred"], bins=np.arange(0, 0.9, 0.2))
...
for y_true in [-1, 1]:
    ...
    if y_true == -1:
        plt.legend(fontsize=12.0, loc="upper right", bbox_to_anchor=(1.1, 1))
    else:
        plt.legend(fontsize=12.0, loc="lower left")  # , bbox_to_anchor=(1.1, 1))
    for l in ax.lines:
        l.set_linewidth(2.0)
        l.set_linestyle("-")
...
for y_true in [-1, +1]:
    ...
    plt.xticks(
        np.arange(0, len(agg_df) + 0.6, (len(agg_df) + 0.6) / 5.0),
        [0.0, 0.2, 0.4, 0.6, 0.8],
    )
    plt.savefig(
        output_dir / "lift-by-gender-by-y-pred-y-true-%d.png" % y_true,
        bbox_inches="tight",
    )
    plt.savefig(
        output_dir / "lift-by-gender-by-y-pred-y-true-%d.pdf" % y_true,
        bbox_inches="tight",
    )
    plt.show()
    plt.close()
...
individuals = (
    ...
    .loc[lambda df: df["y_pred"] < 0.4]
    .loc[lambda df: df["y_pred"] > 0.2]
    .loc[lambda df: df["GoodCustomer"] == 1]
)
...
# 'Cost Distribution'
ax = (
    flipset_df.loc[denied_individuals]
...
for g in ["Male", "Female"]:
    ...
    ax = scores.loc[data_df.loc[lambda df: df["Gender"] == g].index].hist(
        bins=25, color="gold", alpha=0.4, figsize=(4, 4)
    )
    ax.spines["right"].set_visible(False)
    ax.spines["top"].set_visible(False)
    ...
    plt.savefig(
        output_dir / "%s_score_distribution_%s.pdf" % (data_name, g),
        bbox_inches="tight",
    )
    plt.close()
