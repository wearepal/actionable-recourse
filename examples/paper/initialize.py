...
settings["file_header"] = "%s/%s_%s%s" % (
    output_dir,
    settings["data_name"],
    settings["method_name"],
    "_".join(settings["method_suffixes"]),
)
settings["audit_file_header"] = "%s%s" % (
    settings["file_header"],
    "_".join(settings["audit_suffixes"]),
)
...
def load_data():
    ...
    if settings["normalize_data"]:
        ...
        data["X_scaled"] = pd.DataFrame(
            scaler.fit_transform(data["X"].to_numpy(dtype=float), data["y"].values),
...
def get_coefficient_df(model_dict, variable_names=None, scaler=None):
    ...
    for k in sorted(model_dict.keys()):
        ...
        coef_vals, intercept_val = undo_coefficient_scaling(
            coefficients=coef_vals, intercept=intercept_val, scaler=scaler
        )
...
def format_gridsearch_df(grid_search_df, settings, n_coefficients, invert_C=True):
    ...
    train_score_df = (
        ...
        .set_index("level_1")
        .assign(
            split_num=lambda df: df.apply(
                lambda x: x["split_num"].replace("_train_score", ""), axis=1
            )
        )
    )
    ...
    test_score_df = (
        ...
        .assign(
            split_num=lambda df: df.apply(
                lambda x: x["split_num"].replace("_test_score", ""), axis=1
            )
        )
    )
    ...
    model_stats_df = model_stats_df.merge(param_df, left_index=True, right_index=True)
    return model_stats_df.assign(n_coefficients=n_coefficients)


def get_flipset_solutions(
    model, data, action_set, mip_cost_type="max", scaler=None, print_flag=True
):
    """
...
def create_data_summary_plot(
    data_df, subplot_side_length=3.0, subplot_font_scale=0.5, max_title_length=30
):
    ...
    f, axarr = plt.subplots(
        n_rows,
        n_cols,
        figsize=(subplot_side_length * n_rows, subplot_side_length * n_cols),
        sharex=False,
        sharey=True,
    )
...
def create_coefficient_path_plot(
    coefs_df, fig_size=(10, 8), label_coefs=True, label_halign="left", **kwargs
):
