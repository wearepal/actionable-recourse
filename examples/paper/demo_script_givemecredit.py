...
sample_weights = raw_df["age"].apply(lambda x: x < 35).map({True: 0, False: 1.0})
...
X_clf, X_audit_holdout, y_clf, y_audit_holdout = train_test_split(X, y, test_size=0.25)
...
for i in range(1):
    print("Iteration %d..." % i)
    X_train, X_test, y_train, y_test = train_test_split(X_clf, y_clf, test_size=0.25)
    ...
    ## train
    clf_full = LogisticRegressionCV(max_iter=1000, Cs=100).fit(X_train, y_train)
    clf_age_limited = LogisticRegressionCV(max_iter=1000, Cs=100).fit(
        X_biased_train, y_biased_train
    )
...
for dataset in ["full", "downsampled"]:
    ...
    for i in denied_individuals:
        ...
        fb = RecourseBuilder(
            ...
            intercept=intercept[dataset] - (np.log(p / (1.0 - p))),
...
# dataset = 'full'
young_individuals = (
    exp_df.loc[lambda df: df["age"] < 30]
    .loc[lambda df: df["y_full_score"] > 0.96]
    .loc[lambda df: df["y_full_score"] < 0.98]
...
full_stats_unstacked = (
    ...
    .dropna()
    .rename(columns={"y_full_score": "score", "y_full_cost": "cost"})
...
age_stats_unstacked = (
    ...
    .dropna()
    .rename(columns={"y_downsampled_score": "score", "y_downsampled_cost": "cost"})
    .assign(**{"Training Run": "Age-limited dataset"})
)

unstacked_combined_set = (
    pd.concat([age_stats_unstacked, full_stats_unstacked])
        ...
        [
            ...
            (
                exp_df.assign(**{"Training Run": "Age-downsampled"})[
                    ...
                    columns={
                        "y_downsampled_cost": "total_cost",
                        "y_downsampled_score": "y_pred",
                    }
                )
            ),
        ]
...
for y_true in [0, 1]:
    for training_run in ["Downsampled", "Full Dataset"]:
        ...
        ax = sns.violinplot(
            ...
            y="total_cost",
            data=(
                combined_data_df.loc[lambda df: df["Training Run"] == training_run].loc[
                    lambda df: df["y_true"] == y_true
                ]
            ),
            linewidth=0.5,
            cut=0,
        ...
        plt.savefig(
            output_dir
            / "2018-11-18__age-bins-by-sample-%s-y-%s.png"
            % (training_run.replace(" ", "-").lower(), y_true),
            bbox_inches="tight",
        )
        plt.savefig(
            output_dir
            / "2018-11-18__age-bins-by-sample-%s-y-%s.pdf"
            % (training_run.replace(" ", "-").lower(), y_true),
            bbox_inches="tight",
        )
        plt.close()
...
if age_cutoff_exploration:
    ...
    if grid_search_age_cutoff:
        ...
        sns.heatmap(
            auc_df.fillna(0).loc[auc_df.index[::-1]],
        )
    ...
    def cross_val_scores_weighted(
        model, X, y, weights, cv=5, metrics=[sklearn.metrics.roc_auc_score]
    ):
        ...
        for train_index, test_index in kf.split(X.index):
            ...
            for i, metric in enumerate(metrics):
                ...
                training_score = metric(
                    y_train, y_pred_train, sample_weight=np.abs(1 - weights_train)
                )
    ...
    plt.xlabel("AUC on Age > x")
    plt.vlines(train_cutoff, *plt.ylim(), linestyles="dashed")
    plt.text(train_cutoff * 1.01, 0.705, "training\ncutoff", horizontalalignment="left")
    ...
    score_and_age = (
        exp_df.assign(c=1)
        .groupby(pd.cut(exp_df["score"], 50))
        .aggregate({"c": "sum", "age": "mean"})
    ...
    flipset_df = flipset_df.assign(
        total_cost=lambda df: df["total_cost"].replace([-np.inf, np.inf], np.nan)
    ).dropna()

    color_var = "total_cost"
    x_axis_var = "age"

    flipset_cost_and_age = flipset_df.assign(c=1).pipe(
        lambda df: df.groupby(pd.cut(df[x_axis_var], 50))
        .aggregate({"c": "sum", color_var: "mean"})
        # redo index
        .reset_index()
