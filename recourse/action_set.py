...
BoundType: TypeAlias = Literal["percentile", "absolute"]
StepType: TypeAlias = Literal["relative", "absolute"]


class Kwargs(TypedDict, total=False):
    custom_bounds: dict[str, tuple[float, float, BoundType]]
    default_bounds: tuple[float, float, BoundType]
...
class ActionSet:
    ...
    _default_step_type: StepType = "relative"

    def __init__(
        self,
        X: pd.DataFrame | npt.NDArray,
        names: list[str] | None = None,
        y_desired: Literal[-1, 1] = 1,
        **kwargs: Unpack[Kwargs],
        ...
        indices = {}
        elements: dict[str, _ActionElement] = {}
    ...
    def __getitem__(
        self,
        index: int | str | list[int] | list[bool] | list[str] | npt.NDArray | slice,
    ) -> "_ActionElement | _ActionSlice":
        if isinstance(index, str):
            ...
            if isinstance(index[0], int):
                ...
                raise IndexError("list element must be bool, str, int")
    ...
    @check_flag.setter
    def check_flag(self, flag: bool) -> None:
    ...
    def feasible_grid(
        ...
        if len(self.constraints) > 0:
            ...
            subset_limit_names = self.constraints.constrained_names(
                constraint_type=SubsetLimitConstraint
            )
            for n in subset_limit_names:
                j = self._names.index(n)
                output[n] = self._elements[n].feasible_values(
                    x[j], return_actions, return_percentiles, drop_suboptimal=False
                )
...
#### Constraints ####
class SubsetLimitConstraint(NamedTuple):
    id: str
    names: list[str]
    indices: list[int]
    lb: int
...
class _ActionConstraints:
    ...
    def __init__(self, names: list[str]):
        ...
        self._id_counter = 0
        self._constraints: dict[str, SubsetLimitConstraint] = {}
    ...
    def constrained_names(
        self, constraint_type: type[SubsetLimitConstraint] | None = None
    ) -> set[str]:
        """
    ...
    def add_subset_limit_constraint(
        self, names: list[str], lb: int = 0, ub: int = 1, id: str | None = None
    ) -> str:
        """
        ...
        # add constraint
        self._constraints[id] = SubsetLimitConstraint(
            id=id, names=names, indices=indices, lb=lb, ub=ub
        )
...
class _ActionElement:
    ...
    def __init__(
        ...
        name: str,
        values: npt.NDArray,
        bounds: tuple[float, float] | tuple[float, float, BoundType] | None = None,
        variable_type: type | None = None,
        actionable: bool = True,
        step_type: Literal["relative", "absolute"] = "relative",
        step_direction: Literal[+1, 0, -1] = 0,
    ...
    def _check_rep(self, check_flag: bool = True):
        """
    ...
    @actionable.setter
    def actionable(self, flag: bool) -> None:
    ...
    @property
    def bounds(self) -> "_BoundElement":
        return self._bounds

    @bounds.setter
    def bounds(
        self,
        b: "_BoundElement | list[int] | tuple[float, float] | tuple[float, float, BoundType] | None",
    ):
        ...
        assert isinstance(
            b, _BoundElement
        ), "bounds must be a list/tuple of the form (lb, ub) or (lb, ub, bound_type)"
        self._bounds = b
    ...
    def flip_direction(self, flip_direction: float | npt.NDArray):
    ...
    @property
    def step_direction(self) -> float | int | bool | npt.NDArray:
        return self._step_direction

    @step_direction.setter
    def step_direction(self, step_direction: float | int | bool | npt.NDArray) -> None:
        assert np.isfinite(step_direction), "step_direction must be finite"
        self._step_direction = np.sign(step_direction)

    @property
    def step_size(self) -> float | int | bool | npt.NDArray:
        return self._step_size

    @step_size.setter
    def step_size(self, s: float | int | bool | npt.NDArray) -> None:
...
class _BoundElement:
    ...
    _valid_variable_types: set[type] = {bool, int, float}
    _valid_bound_types: set[BoundType] = {"absolute", "percentile"}
    _valid_bound_codes: dict[str, BoundType] = {"a": "absolute", "p": "percentile"}

    def __init__(
        self,
        bound_type: BoundType = "absolute",
        lb: float | np.floating | None = None,
        ub: float | np.floating | None = None,
        ...
        if bound_type == "absolute":
            if lb is None:
                assert (
                    values is not None
                ), "If using 'absolute' bounds without a specified lower bound, please pass in data."
                lb = np.min(values)
            else:
            ...
            if ub is None:
                assert (
                    values is not None
                ), "If using 'absolute' bounds without a specified upper bound, please pass in data."
                ub = np.max(values)
            else:
...
class _ActionSlice:
    ...
    def __init__(self, action_elements: list[_ActionElement]):
...
#### Helper Functions ####
def _check_variable_names(names: list[str]) -> bool:
    """
...
T = TypeVar("T", bound=str | bool | int | float)


def _expand_values(value: npt.NDArray | list[T] | T, m: int) -> list[T]:
...
def tabulate_actions(action_set: ActionSet) -> str:
    """
...
def texify_action_set(action_set: ActionSet) -> str:
    """
    :param action_set: ActionSet object
    :return: formatted latex table summarizing the action set for publications
    """
