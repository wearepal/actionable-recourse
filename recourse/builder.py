...
EnumerationType: TypeAlias = Literal["mutually_exclusive", "distinct_subsets"]
...
class RecourseBuilder(object):
    ...
    @property
    def mip_display(self) -> bool | None:
    ...
    def _get_mip_build_info(self, cost_function_type="percentile", validate=True):
        ...
        actions, percentiles = self._action_set.feasible_grid(
            x=self._x,
            return_actions=True,
        ...
        for n, a in actions.items():
            if len(a) >= 2:
                ...
                indices["action_off_names"].append(
                    info["action_ind_names"][0]
                )  ## the indices of variables that indicate that the feature is "off", i.e. no actions are taken on that feature.
            ...
            try:
                ...
                if self.mip_cost_type == "max":
                    if not np.isclose(info["cost"], np.max(info["costs"]), rtol=1e-4):
                        warnings.warn(
                            "numerical issue: max_cost is %1.2f but maximum of cost[j] is %1.2f"
    ...
    def populate(
        ...
        """
        assert self._mip is not None, "must first initialize recourse IP"
        assert (isinstance(total_items, int) and total_items >= 1) or (
            isinstance(total_items, float) and total_items == float("inf")
        )
        ...
        if self.print_flag:
            print(
                "obtained %d items in %1.1f seconds"
                % (k, time.process_time() - populate_start_time)
            )
...
class _RecourseBuilderCPX(RecourseBuilder):
    ...
    def __init__(self, action_set: ActionSet, x=None, **kwargs):
        """
        :param x: vector of input variables for person x
        :param intercept: intercept value of score function
        :param coefs: coefficients of score function
...
class _RecourseBuilderPythonMIP(RecourseBuilder):
    def __init__(self, *args, **kwargs):
        ...
        self._set_mip_display = (
            lambda mip, display_flag: True
        )  # _set_mip_display(self, mip, display)
    ...
    def build_mip(self):
        ...
        score_with_actions = xsum(
            mip.a[a_j] * indices["coefficients"][j]
            for j, a_j in enumerate(indices["action_var_names"])
        )
        ...
        for info in build_info.values():
            ...
            action_val = xsum(
                mip.u[info["action_ind_names"][k]] * info["actions"][k]
                for k in range(len(info["actions"]))
            )
            mip += (
                action_val == mip.a[info["action_var_name"][0]],
                "set_a[%d]" % info["idx"],
            )

            action_on = xsum(
                mip.u[info["action_ind_names"][k]] for k in range(len(info["actions"]))
            )
        ...
        if cost_type in ("total", "local"):
            ...
            for info in build_info.values():
                ## def cost
                cost_var = xsum(
                    info["costs"][k] * mip.u[info["action_ind_names"][k]]
                    for k in range(len(info["costs"]))
                )
                mip += (
                    mip.c[info["cost_var_name"][0]] == cost_var,
                    "def_cost[%d]" % info["idx"],
                )
                ## set max cost
                mip += (
                    mip.max_cost_var - mip.c[info["cost_var_name"][0]] >= 0,
    ...
    def remove_feature_combination(self):
        ...
        self._mip += xsum(
            self._mip.u[k] * con_vals[i] for i, k in enumerate(feature_off_idxs)
        ) <= float(con_rhs)
        return
